...
// 🏦 Card component for James's Vault
export default function ProductCard2() {
  const { address } = useAccount(); // 
  const vaultId = BigInt(2); // vaultId = 2 → Johns's vault
  ...
  useEffect(() => {
    if (!address) return;
    const savedLikes = localStorage.getItem("johnVaultLikes");
    const savedDislikes = localStorage.getItem("johnVaultDislikes");
    const voted = localStorage.getItem(`johnVaultVoted_${address}`);
  ...
  useEffect(() => {
    localStorage.setItem("johnVaultLikes", likes.toString());
    localStorage.setItem("johnVaultDislikes", dislikes.toString());
  }, [likes, dislikes]);
...
console.log("TVL raw:", TVL);
  ...
  return (
    <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 relative group mt-0">
      <img className="p-8 rounded-t-lg" src="/John1.png" alt="Trader John" />
...
>
  Click to get details about John's vault
</div>
...
>
  YOU CAN JOIN JOHN'S VAULT NOW
  <br />
  <span className="text-lg md:text-xl font-semibold text-gray-700 dark:text-gray-300">
    FOR 20 USD PER MONTH — APY&nbsp;<span className="text-yellow-400">120%</span>
  </span>
</h5>
          ...
          <div className="flex items-center space-x-2 ml-3">
            <button
              onClick={() => {
                ...
                setLikes(likes + 1);
                setUserVoted(true);
                localStorage.setItem(`johnVaultVoted_${address}`, "true");
              }}
      ...
      {currentIncome ? ` ${currentIncome.toFixed(2)} USDT` : "  0"}
        ...
        <div className="flex flex-col gap-0">
          <button
            ...
            className="text-white bg-slate-600 hover:bg-slate-800 font-semibold rounded-lg text-xl px-8 py-4 border-2 border-black"
          >
            JOIN VAULT
          </button>
        ...
        <Modal
          ...
          onConfirm={async () => {
            try {
              ...
              const fee = BigInt(30000000000000); // joinFee JohnVault
              await writeContractAsync({
                functionName: "joinJohnsVault",
                args: [wei],
                value: wei + fee,
              });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-96">
        ...
        <p className="text-gray-800 dark:text-gray-100 mb-2">
          ...
          His strategy is high-return balanced.  James 
          specializes in low cup projects with high growth potential
        </p>
